...
describe('DSLA Protocol Staking Simulation - v1.5 - SLA Not Respected, Reward Not Capped', () => {
  ...
  // POOLS BALANCE
  let P1ProviderPool = '366564';
  let P1UserPool = '198936';
  ...
  // POOLS BALANCE
  let P2ProviderPool = '345635.9328';
  let P2UserPool = '219864.0672';
  ...
  // POOLS BALANCE
  let P3ProviderPool = '322506.23293056';
  let P3UserPool = '242993.76706944';
  ...
  // POOLS BALANCE
  let P4ProviderPool = '296943.288634854912';
  let P4UserPool = '268556.7113651450880';
  ...
  // POOLS BALANCE
  let P5ProviderPool = '268691.122599241648742400';
  let P5UserPool = '296808.877400758351257600';
  ...
  before(async function () {
    ...
    slaRegistry = <SLARegistry>(
      new ethers.Contract(
        slaRegistryDeployment.address,
        slaRegistryDeployment.abi,
        signer
      )
    );
    ...
    sla = <SLA>new ethers.Contract(allSLAs[17], slaDeployment.abi, signer);
    ...
    await dslaToken
      .connect(provider_1_account)
      .mint(provider_1_account.address, toWei(initialStakeBalanceProvider1));
    await dslaToken
      .connect(provider_2_account)
      .mint(provider_2_account.address, toWei(initialStakeBalanceProvider2));
    await dslaToken
      .connect(provider_3_account)
      .mint(provider_3_account.address, toWei(initialStakeBalanceProvider3));
    // Add token to provider accounts
    await dslaToken
      .connect(provider_1_account)
      .approve(sla.address, toWei(initialStakeBalanceProvider1));
    await dslaToken
      .connect(provider_2_account)
      .approve(sla.address, toWei(initialStakeBalanceProvider2));
    await dslaToken
      .connect(provider_3_account)
      .approve(sla.address, toWei(initialStakeBalanceProvider3));
    ...
    await dslaToken
      .connect(user_1_account)
      .mint(user_1_account.address, toWei(initialStakeBalanceUser1));
    await dslaToken
      .connect(user_2_account)
      .mint(user_2_account.address, toWei(initialStakeBalanceUser2));
    await dslaToken
      .connect(user_3_account)
      .mint(user_3_account.address, toWei(initialStakeBalanceUser3));
    // Add token to users accounts
    await dslaToken
      .connect(user_1_account)
      .approve(sla.address, toWei(initialStakeBalanceUser1));
    await dslaToken
      .connect(user_2_account)
      .approve(sla.address, toWei(initialStakeBalanceUser2));
    await dslaToken
      .connect(user_3_account)
      .approve(sla.address, toWei(initialStakeBalanceUser3));
