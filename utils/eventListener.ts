...
const contractCreator = (contract, web3WebsocketProviderUrl) => {
  const { abi, address } = contract;
  try {
    if (/harmony/i.test(process.env.NODE_ENV)) {
      const ws = new WSProvider(web3WebsocketProviderUrl);
    ...
    const web3 = new Web3(
      new Web3.providers.WebsocketProvider(web3WebsocketProviderUrl)
    );
...
// event listener
const eventListener = async (contract, event, web3WebsocketProviderUrl) => {
  console.log(contract);
  const web3Contract = contractCreator(contract, web3WebsocketProviderUrl);
